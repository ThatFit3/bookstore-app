    ...
    <table class="table-auto w-full mt-6">
        <thead class="">
            <tr class="text-left rounded-md bg-gray-100 dark:bg-gray-900">
                ...
                <th class="p-2 border-collapse border border-slate-500">From</th>
            </tr>
        </thead>
        <tbody>
            @foreach($purchases as $purchase)
            @if($purchase->pending == 1 && $purchase->arrive == 1)
            <tr class="text-left">
                ...
                <td class="border-collapse border border-slate-500 p-2 text-center">{{ $purchase->shopItem->shop->shop_name }}</td>                
            </tr>
            @endif
            @endforeach
        </tbody>
